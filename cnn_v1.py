...
model.add(Convolution2D(nb_filters1, (kernel_size[0], kernel_size[1]),
                        padding='same',
                        input_shape=(48, 48, 1)))  # 卷积层1      output(48,48，16)
model.add(Activation('relu'))  # 激活层
model.add(MaxPooling2D(pool_size=pool_size, strides=strides, padding='same'))  # pool 1  output(24,24，16)
# model.add(Dropout(0.4))  # 神经元随机失活
model.add(Convolution2D(nb_filters2, (kernel_size[0], kernel_size[1]), padding='same'))  # 卷积层2  output(24, 24,64)
model.add(Activation('relu'))  # 激活层
model.add(MaxPooling2D(pool_size=pool_size,strides=strides, padding='same'))  # pool 2   output(12,12,64)
model.add(Convolution2D(256, (kernel_size[0], kernel_size[1]), padding='same'))  # 卷积层3  output(12, 12,128)
model.add(Activation('relu'))  # 激活层
model.add(MaxPooling2D(pool_size=(4,4),strides=4, padding='same'))  # pool 2   output(6,6,256)
model.add(Dropout(0.3))  # 神经元随机失活
model.add(Convolution2D(512, kernel_size = 3, padding='valid'))  # 卷积层4  output(1, 1, 512)
model.add(Activation('relu'))  # 激活层
model.add(Flatten())  # 拉成一维数据  512
...
model.fit(x_train, y_train, batch_size=300, epochs=300, validation_data=(x_test, y_test), callbacks=[history])
print('\nTesting ------------')
